...
export default class DragDropTable extends React.Component {
  ...
  onDragEnd(result) {
    ...
    copyPlans.map((plan, id) => {
      if (id == draggableId && id != 0) {
        copyPlans[id].date =
          copyPlans[id].date.split('/')[0] +
          '/' +
          copyPlans[id].date.split('/')[1] +
          '/' +
            ...
            <div key={week.join('') + i}>
              ...
              {week.includes(parseInt(this.state.day)) && (
                <div
                  ...
                  style={this.state.scheduleStyle}
                >
                  <button className={'closedButton'} onClick={this.close}>
                    ×
                  </button>
                  {/* 週の予定を設定するためのコンポーネント */}
